  ...
  <div className="w-full lg:w-1/4 xl:w-1/5 bg-white rounded-2xl shadow-lg p-6 self-start sticky top-8">
    <div className="flex flex-col items-center text-center">
      ...
      <p className="text-gray-500">
        {user.role.name === "USER" ? "พนักงานขาย" : user.role.name}
      </p>
    </div>
...
const ActivityCard = ({
  ...
  const typeColor =
    activity.type === "เข้าพบร้านค้า" ? "bg-purple-500" : "bg-yellow-500";
  return (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
          <div>
            ...
            <p className="font-bold text-gray-900">
              {activity.revenue.toLocaleString()} บาท
            </p>
          </div>
          <div>
            <p className="text-gray-500">รายจ่าย</p>
            <p className="font-bold text-gray-900">
              {activity.cost.toLocaleString()} บาท
            </p>
          </div>
          <div>
            <p className="text-gray-500">รายได้สุทธิ</p>
            <p
              className={`font-bold ${
                isProfitPositive ? "text-green-600" : "text-red-600"
              }`}
            >
              {activity.profit.toLocaleString()} บาท
            </p>
          </div>
          <div>
            <p className="text-gray-500">ระยะเวลา</p>
            <p className="font-bold text-gray-900">{activity.duration} วัน</p>
          </div>
        </div>
      </div>
    </div>
